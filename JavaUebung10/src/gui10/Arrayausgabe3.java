...
public class Arrayausgabe3 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Arrayausgabe3 frame = new Arrayausgabe3();
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(p > 0) {
					...
					zahlen.add(i, Integer.parseInt(hilfe));
				}else {
					zahlen.add(i, Integer.parseInt(eingabe.toString()));
				}
				}
				for(int x = 0; x < a; x++) {
					hilfe2 = hilfe2 + " " + String.valueOf(zahlen.get(x));
				}
				lblVorwärts.setText(hilfe2);
				for(int y = (a-1); y >= 0; y--) {
					hilfe3 = hilfe3 + " " + String.valueOf(zahlen.get(y));
				}
